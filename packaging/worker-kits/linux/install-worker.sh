...
Usage: install-worker.sh <install|repair|uninstall> [options]
  ...
  --pair-only
  --purge-data
  --failure-injection none|after-pair|after-service-registration|before-manifest-write
EOF
}
...
case "$failure_injection" in
  none|after-pair|after-service-registration|before-manifest-write) ;;
...
remove_transaction() {
  ...
  rm -rf --one-file-system -- "$transaction_root"
}
...
if [[ -e "$transaction_root" || -L "$transaction_root" ]]; then
  [[ "$installation_owned_before" -eq 1 ]] || { printf 'Found a worker repair transaction without a valid ownership marker.\n' >&2; exit 1; }
