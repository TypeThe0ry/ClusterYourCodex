...
- The `pids` controller is available and enabled for child cgroups.  The
  `cgroup.kill`, `cgroup.events`, `cgroup.procs`, and `pids.max` files must be
  available on a newly created child (the cgroup-v2 root itself may not expose
  all of those files).
...
`result.json` records native and cleanup exit codes, timestamps, and explicit
residue-verification booleans.  No passwords, private keys, tokens, or worker
credential contents are written to these files.
...
2. writes `cgroup.kill`, waits for `cgroup.procs`/`cgroup.events` to become
   empty, and removes each empty job-owned cgroup with `rmdir`;
