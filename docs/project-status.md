...
- The core-usability candidate merged as PR [#54](https://github.com/TypeThe0ry/ClusterYourCodex/pull/54) at `a103306ec7b2ba8a7b3571fc24317a4876928bf2`. Required checks in run [`34025408108`](https://github.com/TypeThe0ry/ClusterYourCodex/actions/runs/34025408108) all passed: Windows desktop lifecycle and managed worker kits, Windows controller/worker live round trip, Rust workspace tests on Windows/Ubuntu/macOS, native worker kits, MSRV, CodeQL, and dependency checks. The candidate's local evidence also passed the same-host Windows `queued` → `running` → `succeeded` path with heartbeat, logs, artifact, cleanup, and secret scanning.
