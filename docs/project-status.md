...
- The repair branch replaces the unbounded PowerShell task-unregister path with bounded native scheduler deletion, binds exact-path validation and termination to one process handle, reaps runtimes after both registration and rollback restoration, and preserves flattened helper-history evidence. The tagged workflow must pass this acceptance job before the next public preview is published.
