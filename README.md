...
- **Acceptance in progress:** the original Windows 11 ISO matches Microsoft's published hash. A disposable VMware VM reached Windows Setup using a modified, no-prompt test ISO, but Setup requires TPM 2.0; CLI provisioning has not yet produced a usable encrypted vTPM. The clean-VM install/repair/rollback lifecycle remains unverified, and the VM is stopped. See the [VMware evidence](docs/vmware-acceptance-20260920.md). macOS still needs detached-descendant cleanup implementation and native managed-runtime validation before [Issue #3](https://github.com/TypeThe0ry/ClusterYourCodex/issues/3) can close.
